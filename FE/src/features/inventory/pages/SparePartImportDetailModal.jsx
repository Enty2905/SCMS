...
export function SparePartImportDetailModal({ item, onClose }) {
  if (!item) return null
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-3xl rounded-xl bg-white shadow-xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          <h3 className="text-base font-semibold text-slate-950 mb-3">Danh sách vật tư thay thế nhập</h3>
                ...
                {item.items && item.items.length > 0 ? (
                  item.items.map((importedItem, idx) => (
                    <tr key={importedItem.itemId || idx} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3 font-medium text-violet-600">{importedItem.sparePartCode}</td>
                      <td className="px-4 py-3 font-semibold text-slate-950">{importedItem.sparePartName}</td>
                      <td className="px-4 py-3 text-slate-600">{importedItem.sparePartUnit}</td>
