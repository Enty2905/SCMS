...
export function ToolRemoveDamagedModal({ item, onClose, onSuccess }) {
  ...
  async function handleSubmit(event) {
    ...
    if (disposeQty > item.damagedQuantity) {
      setError(`Số lượng hủy không được vượt quá số lượng CCDC đang bị hỏng (${item.damagedQuantity})`)
      return
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-md rounded-xl bg-white shadow-xl flex flex-col">
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <h2 className="text-lg font-semibold text-slate-950">Hủy CCDC hư hỏng</h2>
          <button
        ...
        <div className="px-6 py-5">
          ...
          <div className="mb-4 bg-slate-50 p-3 rounded-md border border-slate-200">
            ...
            <div className="flex gap-4 text-xs text-slate-600">
              ...
              <span>Hư hỏng: <span className="font-bold text-rose-600">{item.damagedQuantity}</span></span>
            </div>
          </div>
          ...
          <form id="disposeForm" onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="mb-1.5 block text-sm font-medium text-slate-700">
                Số lượng hủy <span className="text-rose-500">*</span>
              </label>
              <input
                type="number"
                min="1"
                max={item.damagedQuantity}
                ...
                placeholder={`Tối đa ${item.damagedQuantity}`}
                required
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="VD: Không thể sửa chữa, tiến hành thanh lý..."
                value={note}
              />
            </div>
          </form>
